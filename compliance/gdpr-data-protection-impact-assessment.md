...
|---|---|---|
| Verification data (incl. video) not actually deleted after a decision, contradicting the retention promise and extending exposure of special-category + biometric-adjacent data | **Confirmed to have occurred at least once** (profile VRN-4499, reported 2026-08-24) — see `project_verification_retention_bug` memory. Root cause not yet conclusively confirmed; leading theory is a stale deployed function predating the current deletion logic, not a defect in the logic itself. | Current repo source (`admin-review-profile/index.ts` lines 174–182) correctly fetches `video_path`, removes the storage object, then deletes the row, unconditionally on approve or reject. **Action required**: redeploy this function fresh via the Supabase Dashboard to eliminate any doubt about staleness, then verify a fresh test approval leaves no `profile_verification` row behind. Manually delete VRN-4499's leftover row (and any video object) as a one-off cleanup regardless. |
...
Processing may continue. The design (consent, minimization, immediate deletion, restricted visibility) is sound. The one open item that must be closed out, not just documented, is confirming the deletion actually executes reliably in production — see the redeploy/verify/cleanup action above. This document should be updated once that's confirmed, and the linked memory entry closed out at the same time.
