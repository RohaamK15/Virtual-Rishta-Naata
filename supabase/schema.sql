...
-- ============================================================
-- 11. HOME PAGE ANNOUNCEMENT BANNER
-- ============================================================
-- A native app build already installed on someone's phone can't be patched
-- by a web deploy — every promotional banner or announcement hardcoded into
-- home.html's own markup would need a whole new app release just to turn
-- off later. This single-row table lets admin toggle a banner (e.g. the
-- first-40-members discount) on/off and edit its text instantly, on both
-- the website and every existing app install, with zero rebuild ever.
-- Public SELECT since home.html reads this before anyone signs in; no
-- INSERT/UPDATE/DELETE for anon/authenticated at all — only the
-- admin-set-announcement Edge Function (service role) can change it.
create table if not exists public.site_announcement (
  id text primary key default 'home_banner',
...
create policy "site_announcement_select_anyone" on public.site_announcement
  for select using (true);

revoke insert, update, delete on public.site_announcement from anon, authenticated;
grant select on public.site_announcement to anon, authenticated;

insert into public.site_announcement (id, active, message, link_url, link_text)
values ('home_banner', false, null, null, null)
on conflict (id) do nothing;
