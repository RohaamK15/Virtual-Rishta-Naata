...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { id, name, subject, body, is_html, from_alias } = await req.json();
    ...
    if (id) {
      const { error } = await admin.from("email_template").update({
        name, subject, body, is_html: !!is_html, from_alias: from_alias || null, updated_at: new Date().toISOString(),
      }).eq("id", id);
      if (error) throw error;
    } else {
      const { data, error } = await admin.from("email_template").insert({
        name, subject, body, is_html: !!is_html, from_alias: from_alias || null,
      }).select("id").single();
      if (error) throw error;
      savedId = data.id;
    }
